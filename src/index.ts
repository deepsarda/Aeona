...
for (const config of configs) {
  ...
  manager.on("clusterCreate", (cluster) => {
    ...
    cluster.on("death", function () {
      console.log(
        `[${config.name}]:`.yellow +
          `${colors.red.bold(`Cluster ${cluster.id} died..`)}`
      );
    });
    ...
    cluster.on("error", (e) => {
      console.log(
        `[${config.name}]:`.yellow +
          `${colors.red.bold(`Cluster ${cluster.id} errored ..`)}`
      );
      console.error(e);
    });
    ...
    cluster.on("disconnect", function () {
      console.log(
        `[${config.name}]:`.yellow +
          `${colors.red.bold(`Cluster ${cluster.id} disconnected..`)}`
      );
    });
  ...
  manager.once("debug", (d) =>
    d.includes("[CM => Manager] [Spawning Clusters]")
      ? console.log(`[${config.name}]:`.yellow + d)
      : ""
  );
