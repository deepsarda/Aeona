...
import { SequentialBucket } from './util/SequentialBucket.js';
...
export class RequestHandler {
	...
	userAgent = `DiscordBot (https://github.com/abalabahaha/eris, 0.17.2)`;
			...
			const actualCall = (cb: () => void) => {
				...
				try {
					...
					if (file) {
						...
						if (method === 'GET' || method === 'DELETE') {
							...
							Object.keys(body).forEach(function (key) {
								if (body[key] != undefined) {
									if (Array.isArray(body[key])) {
										body[key].forEach(function (val: string | number | boolean) {
							...
							data = JSON.stringify(body, (_k, v) => (typeof v === 'bigint' ? v.toString() : v));
				...
				req.once('response', (resp) => {
					...
					if (this.latencyRef.lastTimeOffsetCheck < Date.now() - 5000) {
						...
						if (
							this.latencyRef.timeOffset - this.latencyRef.latency >= this.options.latencyThreshold &&
							timeOffset - this.latencyRef.latency >= this.options.latencyThreshold
						) {
							console.warn(
								'warn',
								new Error(
						...
						this.latencyRef.timeOffset =
							this.latencyRef.timeOffset - ~~(this.latencyRef.timeOffsets.shift() / 10) + ~~(timeOffset / 10);
							...
							if ((resp.statusCode || 0) >= 300) {
								if (resp.statusCode === 429) {
									...
									if (delay) {
										setTimeout(() => {
											cb();
											this.request(method, url, body, file, route, true).then(resolve).catch(reject);
										}, delay);
										return;
									} else {
										cb();
									...
									console.error('debug', 'A wild 502 appeared! Thanks CloudFlare!');
									setTimeout(() => {
										this.request(method, url, body, file, route, true).then(resolve).catch(reject);
