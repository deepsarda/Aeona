		...
		case MessageTypes.ChannelPinnedMessage:
			return (
				<DiscordSystemMessage id={`m-${message.id}`} key={message.id + ''} type="edit">
					<Highlight
						color={rgbToHex(
							(await highestRole(bot, message.member?.guildId ? message.member.guildId : BigInt(0), message.member!))
								.color,
						)}
					>
...
export async function JoinMessage(bot: AeonaBot, member: Member | undefined, fallbackUser: User): Promise<string> {
	...
	randomMessage.replace(
		'{user}',
		`<Highlight
    color={${rgbToHex((await highestRole(bot, member?.guildId ? member.guildId : BigInt(0), member!)).color)}}
  >
