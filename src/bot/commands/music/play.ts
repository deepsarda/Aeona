...
export default {
	...
	async execute(client: AeonaBot, ctx: Context) {
		if (!ctx.guild || !ctx.user || !ctx.channel || !ctx.member) return console.log(ctx.guild + ' ' + ctx.channel + ' ' + ctx.user);

		const song = ctx.options.getLongString('song', true);

		if (!client.extras.voiceStates.get(ctx.guildId + "_" + ctx.user.id)) return client.extras.errNormal({
			error: `You're not in a voice channel!`,
			type: 'reply'
		}, ctx);

		const channel = await client.helpers.getChannel(client.extras.voiceStates.get(ctx.guildId + "_" + ctx.user.id)?.channelId!);


		let player = client.extras.player.players.get(ctx.guild.id + "");

		if (player && (channel.id + "" !== player?.voiceChannel)) return client.extras.errNormal({
		...
		if (!player) {
			...
			setTimeout(() => {
				if (channel.type == ChannelTypes.GuildStageVoice) {
					client.helpers.updateBotVoiceState(ctx.guildId!, { channelId: channel.id, suppress: false })
				}
			}, 500)
		}
		...
		switch (res.loadType) {
			...
			case 'SEARCH_RESULT': {
				...
				if (index < 0 || index > max - 1) return client.extras.errNormal({
