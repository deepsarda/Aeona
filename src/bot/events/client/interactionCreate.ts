...
export default async (client: AmethystBot, interaction: Interaction) => {
	...
	if (interaction.type == InteractionTypes.MessageComponent) {
		if (interaction.data?.customId == 'reaction_select') {
			...
			if (interaction.data?.values[0] == 'share-discord' &&  interaction.data?.customId.split("_")[0]== interaction.user.id+"") {
				await client.helpers.createForumThread('1045332279943233667', {
					name: 'Art generated by ' + interaction.user.username + '(' + interaction.member.id + ')',
					autoArchiveDuration: 60,
					content: interaction.message.content,
					file: {
						name: 'image.png',
						blob: await (await fetch(interaction.message.attachments[0].proxyUrl)).blob(),
					},
				});
				await client.helpers.sendInteractionResponse(interaction.id, interaction.token, {
					type: 4,
