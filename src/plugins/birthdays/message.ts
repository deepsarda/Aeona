...
export class Birthdays {
    ...
    async list(
        ...
        const lb = rawBirthdayboard.map(
            (e) => `${bot.config.emotes.normal.birthday} | **<@!${e.User}>** → ${e.Birthday} `,
        );
    ...
    async set(
        ...
        if (!day || !month) return bot.extras.errUsage({
    ...
    @SimpleCommand({
        ...
        aliases: ['bday setchannel', 'birthdays setchannel','bday channel', 'birthdays channel' , 'bday setchannel', 'birthdays setchannel'],
        description: 'Set your birthday 🎂',
    })
    @Guard(
        RateLimit(TIME_UNIT.seconds, 30, {
