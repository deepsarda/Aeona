...
export default {
  ...
  async execute(client: AeonaBot, ctx: Context) {
    ...
    async function sendMessage() {
      const data = await chatbotChannel.find({ Guild: `${ctx.guild!.id}` });
      ...
      if (data.length > 0)
        comp.addSelectComponent(
          ...
          'Edit/Delete the settings for your ChatBot Systems',
        );
      else
        comp.addSelectComponent(
          'Edit/Delete a system.',
        ...
        .then(async (interaction) => {
          ...
          if (interaction.data?.customId == 'autocreate') {
            ...
            await client.helpers.sendInteractionResponse(interaction.id, interaction.token, {
              type: 4,
              data: {
                content: `I have successfully setup <#${channel.id}> as a chatbot channel.`,
                flags: 1 << 6,
              },
            });
            ...
            while (!success) {
              ...
              if (message.mentionedChannelIds && message.mentionedChannelIds.length > 0) {
                ...
                await client.helpers.sendFollowupMessage(interaction.token, {
                  content: `I have successfully setup <#${message.mentionedChannelIds[0]}> as a chatbot channel.`,
                  flags: 1 << 6,
                });
              } else {
