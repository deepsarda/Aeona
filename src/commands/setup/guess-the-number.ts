...
export default {
  ...
  async execute(client: AeonaBot, ctx: Context) {
    ...
    async function sendMessage() {
      const data = await GTN.find({ Guild: `${ctx.guild!.id}` });
      ...
      if (data.length > 0)
        comp.addSelectComponent(
          ...
          'Edit/Delete the settings for your Guess The Number Systems',
        );
      else
        comp.addSelectComponent(
          'Edit/Delete a system.',
          ...
          if (interaction.data?.customId == 'autocreate') {
            ...
            await client.helpers.sendInteractionResponse(
              ...
              interaction.token,
              {
                type: 4,
                data: {
                  content: `I have successfully setup <#${channel.id}> as a guess the number channel.`,
                  flags: 1 << 6,
                },
              },
            );
            ...
            while (!success) {
              ...
              if (
                ...
                await client.helpers.sendFollowupMessage(interaction.token, {
                  type: 4,
                  data: {
                    content: `I have successfully setup <#${message.mentionedChannelIds[0]}> as a guess the number channel.`,
                    flags: 1 << 6,
                  },
                });
              } else {
