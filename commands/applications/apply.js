...
module.exports = {
    ...
    execute: async (message, args, bot, prefix) => {
        ...
        if (!app.appLogs) return message.channel.sendError({'title': 'Applications', 'description': 'It seems that this server has no channel setup for applications.'});

        let channel = await bot.channels.fetch(app.appLogs);

        if (!channel) return message.channel.sendError({'title': 'Applications', 'description': 'It seems that this server application log channel does not exist anymore.'});
