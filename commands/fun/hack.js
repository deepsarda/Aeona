...
module.exports = class extends Command {
  ...
  async run(message) {
    ...
    message.channel
      .send(`${language.hack11} "${member.user.username}" ${language.hack12}`)
      .then(async (msg) => {
        ...
        setTimeout(async function () {
          await msg
            .edit(
              `[▖] Last DM: "${
                lastdm[Math.floor(Math.random() * lastdm.length)]
              }"`
            )
            .catch(() => {});
        }, 6000);
        ...
        setTimeout(async function () {
          await msg
            .edit(`[▖] IP address: \`127.0.0.1:${darkrandom.int(100, 9999)}\``)
            .catch(() => {});
        }, 20000);
        setTimeout(async function () {
        ...
        setTimeout(async function () {
          await msg
            .edit(`${language.hack17} ${member.user.username}`)
            .catch(() => {});
        }, 32000);
        setTimeout(async function () {
          await message.channel.send(`${language.hack18}`).catch(() => {});
        }, 34000);
      });
  }
};
