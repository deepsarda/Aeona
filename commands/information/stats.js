...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      name: "stats",
      aliases: ["s", "botinfo"],
      description: "Displays Aeonas Statistics",
  ...
  async run(message, client) {
    ...
    const shardGuildCounts = await this.client.shard.fetchClientValues(
      "guilds.cache.size"
    );
    const totalGuildCount = shardGuildCounts.reduce(
      (total, current) => total + current
    );
    const shardUserCounts = await this.client.shard.fetchClientValues(
      "users.cache.size"
    );
    const totalUserCount = shardUserCounts.reduce(
      (total, current) => total + current
    );
    const shardChannelCounts = await this.client.shard.fetchClientValues(
      "users.cache.size"
    );
    const totalChannelCount = shardUserCounts.reduce(
      (total, current) => total + current
    );

    let rss = client.shard
      ? await client.shard.broadcastEval("process.memoryUsage().rss")
      : process.memoryUsage().rss;
    if (rss instanceof Array) {
      rss = rss.reduce((sum, val) => sum + val, 0);
    }
    let heapUsed = client.shard
      ? await client.shard.broadcastEval("process.memoryUsage().heapUsed")
    ...
    const tech = stripIndent`
      ...
      ${language.AeonaCommands} -- ${message.client.commands.size}
      Aliases -- ${message.client.aliases.size}
    `;

    const devs = stripIndent`
     -------
     ${language.AeonaOwners}
    • Peter_#4444

     ${language.AeonaDevelopers}
    • Peter_#4444

     ${language.AeonaContributor}
    • GhostSlayer#0001
    • ΛCЄ#0001
    -------
    `;
