...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    if (!args || args.length < 1) {
      ...
      embed.setTitle(` ${emojis.music} - Music`);
      embed.setDescription(
        this.client.commands
      ...
      embed.setTitle(` ${emojis.tanktactics} - tanktactics`);
      embed.setDescription(
        this.client.commands
          .filter((cmd) => cmd.category.toLowerCase() === "tanktactics")
          .map(
            (cmd) =>
              `${
                cmd.disabled || disabledCommands.includes(cmd.name || cmd)
      ...
      embed.setDescription(
        this.client.commands
          .filter((cmd) => cmd.category.toLowerCase() === "nsfw")
          .map(
            (cmd) =>
              `${
                cmd.disabled || disabledCommands.includes(cmd.name || cmd)
