...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    message.channel.send(
      `The following Command has been disabled Since The tiktok API is not working. We are working on making our own API and bringing the command back!`
    );
    ...
    try {
      ...
      if (user.user.signature == "") {
        const userbe = new discord.MessageEmbed()
          ...
          .addField(
            `${language.tiktok6}`,
            numformat(`${user.stats.followerCount}`),
            true
          )
          .addField(
            `${language.tiktok7}`,
            numformat(`${user.stats.followingCount}`),
            true
          )
          .addField(
            `${language.tiktok8}`,
            numformat(`${user.stats.heartCount}`),
            true
          )
          .setFooter(
            `${language.tiktok9} ${message.author.username}`,
        ...
        const userbd = new discord.MessageEmbed()
          ...
          .setFooter(
            `${language.tiktok9} ${message.author.tag}`,
            message.author.displayAvatarURL({ dynamic: true })
          );
