  ...
  async run(message, args) {
    ...
    const ticketFetch = await ticketSchema.findOne(
      ...
      async (err, db) => {
        ...
        if (!message.member.roles.cache.has(ticketRole.id))
          return message.channel.send(
            new discord.MessageEmbed()
              ...
              .setDescription(
                language.claimNotHaveRole.replace("{roleName}", ticketRole.name)
              )
          );
        ...
        message.channel
          .updateOverwrite(message.author, {
            VIEW_CHANNEL: true,
          })
          .catch((err) => {
        ...
        message.channel
          .updateOverwrite(everyone, { VIEW_CHANNEL: false })
          .catch((err) => {
            message.channel.send(
              new discord.MessageEmbed()
                .setColor(client.color.red)
