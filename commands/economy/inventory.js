...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    await inventory.findOne({ user: message.author.id }, async (err, data) => {
      ...
      if (data) {
        inv = Object.keys(data.Inventory)
          .map((key) => {
      ...
      let i = shop.find((val) =>
        val.item.toLowerCase().includes(equip.toLowerCase())
      );
      if (
        !data ||
        data.Inventory === null ||
      ...
      message.channel.send(
        successEmbed
          ...
          .setDescription(`You have hunted **{TimeYouHunted}** times`)
          .addField("**Inventory**", `${inv} `, true)
          .addField(
            "**Stats**",
            `You have found **{TotalAnimals}** animals\nGun Equipped: **${i.emoji} ─ ${i.item}**`,
            true
          )
      );
    });
