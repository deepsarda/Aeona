...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      name: "logs",
      description: "See all the logs in your inventory.",
      category: "economy",
      cooldown: 3,
      usage: "",
    });
  ...
  async run(message, args, bot) {
    ...
    let logs = await bot.economy.getLogs(message.member);
    if (!logs) {
      util.error({
        msg: message,
        title: "Logs",
        description: "You don't have any logs.",
      });
      return;
    }
    let pages = [];
    for (let i = 0; i < logs.length; i++) {
      ...
      pages[pages.length - 1].push(logs[i]);
    }
    if (pages.length == 0) {
      util.error({
        msg: message,
