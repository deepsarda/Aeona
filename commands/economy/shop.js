  ...
  async run(message, args) {
    ...
    if (!args[0]) {
      return message.channel.send(
        errorEmbed.setDescription("Select category: gun")
      );
    }

    let itemToBuy = args.slice(0).join(" ").toLowerCase();

    let validCategory = !!gun.find((val) =>
      val.category.includes(itemToBuy.toLowerCase())
    );
    if (validCategory) {
      let emoji;
      let nothing;
      await inventory.findOne(
        ...
        async (err, data) => {
          if (
            gun
              .find((val) =>
                val.item.toLowerCase().includes(itemToBuy.toLowerCase())
              )
              .category.includes(itemToBuy)
          ) {
            const gunList = gun
              .map((val) => {
                let value = gun.find((v) =>
                  v.item.toLowerCase().includes(val.item.toLowerCase())
                );
                let str = `${emoji} | **${value.emoji} ${
                  value.item
                }** ─ ${numberWithCommas(
                  value.price
                )} <:coin:822159088221814836>\n${value.description}`;
                ...
                if (data) {
                  ...
                  if (
                    data.Inventory[value.item] === undefined ||
                    !data.Inventory[value.item]
                  ) {
                    emoji = "<:wrong:822376943763980348>";
                  }

                  if (
                    data.Inventory[value.item] == "1" ||
                    data.Inventory[value.item] == Number(1)
                  ) {
            ...
            return message.channel.send(
              successEmbed.setTitle("**Gun Shop**").setDescription(gunList)
            );
          }
        }
      );
    ...
    const validItem = !!gun.find((val) =>
      val.item.toLowerCase().includes(itemToBuy.toLowerCase())
    );
    if (!validItem)
      return message.channel.send(errorEmbed.setDescription("Not valid item"));

    if (gun.length === 0)
      return message.channel.send(errorEmbed.setDescription("Shop is empty"));

    if (validItem) {
      const i = gun.find((val) =>
        val.item.toLowerCase().includes(itemToBuy.toLowerCase())
      );
      await inventory.findOne(
        { user: message.author.id },
