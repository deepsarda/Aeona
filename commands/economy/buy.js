...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    const buyable = items.find((val) =>
      val.item.toLowerCase().includes(itemToBuy)
    ).buyable;

    if (buyable === false)
      return message.channel.send(
        errorEmbed.setDescription("You cant buy this item")
      );
    ...
    if (!userBalance) {
      ...
      return message.channel.send(
        errorEmbed.setDescription("Not enough money1")
      );
    }
    ...
    await inventory.findOne(params, async (err, data) => {
      if (data) {
        ...
        if (!hasItem) {
          ...
          return message.channel.send(
            errorEmbed.setDescription("You already own this item")
          );
        ...
        await userBalance.updateOne({
          balance: userBalance.balance - itemPrice,
        });
