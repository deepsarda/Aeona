...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      name: "fishes",
      description: "See all the fishes in the player inventory.",
      category: "economy",
      cooldown: 3,
      usage: "",
    });
  ...
  async run(message, args, bot,prefix='+' ) {
    ...
    let fishes = await bot.economy.getFishes(message.member);
    if (!fishes) {
      util.error({
        msg: message,
        title: "Fishes",
        description: "You don't have any fishes.",
      });
      return;
    }
    let pages = [];
    for (let i = 0; i < fishes.length; i++) {
      ...
      pages[pages.length - 1].push(fishes[i]);
    }
    if (pages.length == 0) {
      util.error({
        msg: message,
