...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    let muteRole = await message.guild.roles.cache.get(
    ...
    if (
      ...
      if (logging.moderation.mute_action === "2") {
        dmEmbed = `${message.client.emoji.fail} You've been muted in **${
          message.guild.name
    ...
    const noEveryone = mentionedMember.roles.cache.filter(
      (r) => r.name !== "@everyone" && !r.managed && r.id !== muteRole.id
    );
    ...
    if (logging && logging.moderation.remove_roles === "true") {
      for (const role of noEveryone) {
        await mentionedMember.roles
          .remove(role, [
            `Reason: Mute Command - ${reason} | Responsible User: ${message.author.tag}`,
          ])
          .catch(() => {});
    ...
    await mentionedMember.roles
      .add(muteRole.id, [
        `Mute Command - ${reason} / Responsible User: ${message.author.tag}`,
      ])
      .catch(() => {});
    ...
    setTimeout(async () => {
      ...
      await mentionedMember.roles
        .remove(muteRole.id, [
          `Mute Command / Responsible User: ${message.author.tag}`,
        ])
        .catch(() => {});
              ...
              if (
                ...
                if (logging.moderation.mute == "true") {
                  ...
                  const logEmbedm = new MessageEmbed()
                    .setAuthor(
                      `Action: \`Un Mute\` | ${mentionedMember.user.tag} | Case #${logcase}`,
                      mentionedMember.user.displayAvatarURL({ format: "png" })
                    )
                    .addField("User", mentionedMember, true)
