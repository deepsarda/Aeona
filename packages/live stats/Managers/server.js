...
class Server extends Events {
  ...
  _buildRoute() {
    this.app.get(`/dev/${this.config.login_path || ""}`, (req, res) => {
      res.redirect(
        `https://discord.com/api/oauth2/authorize?client_id=${
          this.config.bot.client_id
        }&redirect_uri=${
          this.config.redirect_uri
        }&response_type=code&scope=${this.config.scope.join("+")}`
      );
    });

    this.app.get("/dev/login", async (req, res) => {
      try {
        ...
        if (!code) return res.redirect("/dev");
    ...
    this.app.get("/dev/killShard", (req, res) => {
      try {
        ...
        const code = req.query.code;
        if (!this.code.checkSession(code))
          return res.end(`AUTHENTICATION FAILED | RELOAD & LOGIN AGAIN`);
    ...
    this.app.post("/dev/stats", (req, res) => {
      try {
        ...
        const authKey = Buffer.from(this.config.authorizationkey).toString(
          "base64"
        );
  ...
  _checkIfShardAlive(shardID) {
    ...
    if (!data) return;
    const diff = Number(
      data.lastupdated + this.config.markShardasDeadafter - 1000
    );
  ...
  _validateOptions() {
    if (!this.config.bot)
      throw new Error(
        `Missing Bot Parameters such as redirect_uri, client_secret...`
      );
    if (!this.config.bot.client_id)
      throw new Error(`Missing Parameter: client_id has not been provided`);
    if (!this.config.bot.client_secret)
      throw new Error(`Missing Parameter: client_secret has not been provided`);

    if (!this.config.stats_uri)
      throw new Error(`Missing Parameter: stats_uri has not been provided`);
    if (!this.config.redirect_uri)
      throw new Error(`Missing Parameter: redirect_uri has not been provided`);
    if (!this.config.owners)
      throw new Error(`Missing Parameter: owners has not been provided`);

    if (!this.config.scope) this.config.scope = ["identify"];

    if (!this.config.postinterval) this.config.postinterval = 2500;
    if (!this.config.markShardasDeadafter)
      this.config.markShardasDeadafter = 10000;
    if (!this.config.loginExpire) this.config.loginExpire = 60000 * 15;
    if (!this.config.login_path) this.config.login_path = "";

    if (this.config.postinterval > this.config.markShardasDeadafter)
      throw new Error(
        `Post Interval can not be bigger than the "maskShardasDeadafter" Argument`
      );
  }
}
