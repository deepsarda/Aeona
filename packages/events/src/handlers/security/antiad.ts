...
export default (client: AeonaBot) => {
  client.on(
    'messageCreateNoBots',
    async (client: AeonaBot, message: Message) => {
          ...
          if (data) {
            if (data.AntiInvite == true) {
              ...
              if (code) {
                Schema2.findOne(
                  { Guild: message.guildId },
                  async (err: any, data2: { Channels: bigint[] }) => {
                    if (data2) {
                      if (
          ...
          if (data) {
            if (data.AntiInvite == true) {
              ...
              if (code) {
                Schema2.findOne(
                  { Guild: newMessage.guildId },
                  async (err: any, data2: { Channels: string | any[] }) => {
                    if (data2) {
                      if (
                      ...
                      const error = new AmethystEmbed()
                        .setTitle(
                          `${client.extras.emotes.normal.error} Moderator`,
                        )
                        .setAuthor(
