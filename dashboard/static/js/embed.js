...
$(document).ready(() => {
  ...
  function updateEmbed(embed) {
    ...
    if (embed.url) {
      ...
      $(".embed-inner").append(
        `<div class="embed-title">${embed.title
          .match(/.{1,46}/g)
          .join("<br/>")}</div>`
      );
    }
    ...
    if (embed.description) {
      $(".embed-inner").append(
        `<div class="embed-description">${embed.description
          .match(/.{1,46}/g)
          .join("<br/>")
          .replace(/{user}/g, `<b style="color: red">{User}</b>`)
          .replace(/{user_tag}/g, `<b style="color: red">{user_tag}</b>`)
          .replace(/{guild}/g, `<b style="color: red">{guild}</b>`)
          .replace(
            /{memberCount}/g,
            `<b style="color: red">{memberCount}</b>`
          )}</div>`
      );
    }
    ...
    if (embed.author.name) {
      $(".embed-title").before(
        `<div class="embed-author"><a class="embed-author-name" href="${
          embed.author.url
        }">${embed.author.name.match(/.{1,46}/g).join("<br/>")}</a></div>`
      );
  ...
  function addWarning(item, type, message) {
    ...
    if ($(`#${type}-feedback`).length === 0) {
      item.after(
        `<div class="form-control-feedback" id="${type}-feedback">${message}</div>`
      );
    }
  }
