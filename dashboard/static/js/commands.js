window.onload = () => {
  ...
  if (window.location.search.includes("min=true"))
    document.body.classList.remove("zoomed"); // ?min=true for minimized mode
  if (window.self === window.top)
    cmdsBoxInner.querySelector("header").style.display = "block";
  else {
    newTab.style.display = "block";
    if (window.top.location.origin === window.self.location.origin)
      window.top.document.querySelector(".loader").style.display = "none";
  }
  if (window.location.hash) {
    ...
    if (document.querySelector(id)) {
      if (document.querySelector(`.cmdFilter.${id.slice(1)}Selector`)) {
        cmdFilters
          .querySelector(".cmdFilter.active")
          .classList.remove("active");
        document
          .querySelector(`.cmdFilter.${id.slice(1)}Selector`)
          .classList.add("active");
      }
    }
    cmdFilters.style.transform =
      (cmdsBoxInner.scrollTop > 100 &&
        `translateY(${cmdsBoxInner.scrollTop - 55}px)`) ||
      (cmdsBoxInner.scrollTop <= 2980 && "translateY(55px)");
  }
  const ifrNum = (window.self !== window.top && 110) || 0;

  cmdsBoxInner.addEventListener(
    "scroll",
    () =>
      (cmdFilters.style.transform =
        (cmdsBoxInner.scrollTop > 100 &&
          `translateY(${cmdsBoxInner.scrollTop - 55 + ifrNum}px)`) ||
        (cmdsBoxInner.scrollTop <= 2980 && "translateY(55px)"))
  );
  ...
  document.querySelectorAll(".cmds .cmd:not(.ccmd)").forEach((elem) => {
    elem.addEventListener("click", (clicked) => {
      if (
        ...
        clicked.target.classList.contains("cmd")
      ) {
  ...
  document
    .querySelectorAll(".emojiImage")
    .forEach(
      (e) => (e.src = `https://cdn.discordapp.com/emojis/${e.classList[1]}.png`)
    );

  window.matchMedia("(max-width: 1215px)").addEventListener("change", (e) => {
    if (e.matches) {
      (cls = document.body.classList),
        (win = document.querySelector(".win2")),
        (inner = win.querySelector(".inner"));
