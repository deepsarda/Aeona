...
module.exports = class extends Event {
  ...
  async run() {
    ...
    console.log("-----------");
    logger.info(
      `Shard ${this.client.shard.ids - 1 + 2}/${
        ...
        for (const muteDoc of muteArray) {
          if (Date.now() >= Number(muteDoc.length)) {
            ...
            if (member) {
              ...
              if (logging) {
                let muteRole = await guild.roles.cache.get(
                  logging.moderation.mute_role
                );

                await member.roles
                  .remove(muteRole.id, [`Mute Command | Duration Expired`])
                  .catch(() => {});
                ...
                if (logging && logging.moderation.remove_roles === "true") {
                  for (const role of muteDoc.memberRoles) {
                    ...
                    if (roleM) {
                      await member.roles
                        .add(roleM, ["Mute Command, Mute duration Expired."])
                        .catch(() => {});
                  ...
                  if (channel) {
                    if (logging.moderation.mute == "true") {
                      ...
                      const logEmbedm = new MessageEmbed()
                        .setAuthor(
                          `Action: \`Un Mute\` | ${member.user.tag} | Case #${logcase}`,
            ...
            if (resultsArr[0][1] == resultsArr[1][1]) {
              ...
              if (msg) {
                let embed = new MessageEmbed()
                  .setTitle(poll.title)
                  .setDescription(
                    `${language.poll7} ${resultsArr[0][0]} ${language.with} ${
                      resultsArr[0][1] - 1
                    } ${language.votesss} `
                  )
                  .setColor(`GREEN`)
